...
describe('sttr_build parser', () => {
  test('name', () => {
    const xml = fs.readFileSync(path.join(__dirname, '__mocks__', 'demo.dmn')).toString();
  ...
  test('conclusion description', () => {
    const xml = fs.readFileSync(path.join(__dirname, '__mocks__', 'with-conclusion-description.dmn')).toString();
    const config = parser(xml);
    const mapping = {
  ...
  test('questions', () => {
    ...
    const questions = {
      ...
      'vraag-aanbouwmetvergunning': {
        type: 'boolean',
        text: 'Ga je een aanbouw maken waar je een vergunning voor moet aanvragen?',
      },
    };
    const decisions = {
      'decision-pad-1': {
      ...
      'decision-pad-2': {
        requiredInputs: ['#input-id-wietplantage', '#input-id-aanbouwmetvergunning'],
        decisionTable: {
          rules: [
            {
              inputs: [false, true],
  ...
  test('geo', () => {
    const xml = fs.readFileSync(path.join(__dirname, '__mocks__', 'geo.dmn')).toString();
