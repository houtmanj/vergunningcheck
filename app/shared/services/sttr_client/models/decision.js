...
const { collectionOfType, uniqueFilter } = require('../util');
...
class Decision {
  ...
  constructor(id, inputs, rules) {
    ...
    if (!Array.isArray(inputs) || (!collectionOfType(inputs, 'Decision') && !collectionOfType(inputs, 'Question'))) {
      throw Error(`'inputs' must be an array of Question's or Decision's, got: ${inputs} for Decision: ${id}.`);
    }
    if (!Array.isArray(rules) || !collectionOfType(rules, 'Rule')) {
      throw Error(`'rules' must be an array of type 'Rule' (got ${JSON.stringify(rules)})`);
    }
    this._id = id;
    this._inputs = inputs;
    this._rules = rules;
  ...
  getDecisiveInputs() {
    const values = this._inputs.map(({ answer }) => answer);
    return this._rules.flatMap(rule => rule.evaluateNew(values).map(index => this._inputs[index])).filter(uniqueFilter);
  }
   ...
   */
  getQuestions() {
    return this._inputs.filter(input => input.constructor.name === 'Question');
  }
