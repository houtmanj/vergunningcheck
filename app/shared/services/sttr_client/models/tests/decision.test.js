...
beforeEach(() => {
  ...
  const question = new Question('abc', 'boolean', 'Are you planning something big?');
  const decision = new Decision('fake-id', [question], [yes, no]);

  const conclusionYes = new Rule(['Something big'], 'You need this permit.');
  const conclusionNo = new Rule(['Something small'], "You don't need this permit.");

  const conclusionDecision = new Decision('dummy', [decision], [conclusionYes, conclusionNo]);
  mock = {
    rules: { yes, no },
    question,
    decision,
...
describe('Decision', () => {
  test('id', () => {
    ...
    expect(() => new Decision(3, [mock.question], [mock.yes, mock.no])).toThrow("'id' must be a String");
  });
  ...
  describe('for Questions', () => {
    ...
    test('getMatchingRule', () => {
      // XXX assert we only get the FIRST or LAST if there are multiple???
