...
describe('Checker recursive', () => {
  test('initialization', () => {
    ...
    const d1 = new Decision('a', [q1], [new Rule([true], 'fun!'), new Rule([false], 'boring')]);
    const d2 = new Decision('b', [q1, q2], [new Rule([true, false], 'non local'), new Rule([true, true], 'local')]);
    ...
    question = checker.rewindTo(0);
    question.setAnswer(false);
    expect(checker.getOutputByDecisionId('dummy')).toBe('Maybe you should move?');
  });
});
